...
- Web服务: Nginx（开机自启）
- 后端: Node.js + pm2 进程守护（端口 3003，开机自启）
...
## 备份配置
- 每日 23:00 自动双重备份（本地代码 + 服务器数据）
- 备份路径：`/Users/fengshisan/Desktop/筹建系统代码备份/YYYY-MM-DD/`
- 备份内容：`进度管理系统.html`、`data.json`、`auth.json`
- SSH 免密密钥：`/Users/fengshisan/.ssh/store_backup_key`
- 自动清理：只保留最近30天备份
- 自动化任务 ID：`automation-2`
...
- 2026-04-27: 筛选下拉框改为多选（MultiSelect组件），覆盖区域门店统计、门店周期统计、预估开业日历共6个下拉框
- 2026-05-08: 筹建天数计算起点从"确认门店测量时间"开始（跳过选址阶段）
- 2026-05-08: 配置每日23:00自动双重备份 + SSH免密登录
